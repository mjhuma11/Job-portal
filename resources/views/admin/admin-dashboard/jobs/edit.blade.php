...
@section('title', 'Edit Job Post - Admin Panel')
        ...
        <div class="flex-1 p-6">
            ...
            <div class="mb-6">
                <div class="flex items-center space-x-2 text-sm text-gray-500 mb-2">
                    ...
                    <span class="text-teal-600">Edit Job Post</span>
                </div>
                <h1 class="text-3xl font-bold text-gray-900">Edit Job Post</h1>
                <p class="text-gray-600 mt-1">Update job posting details</p>
            </div>

            <!-- Job Edit Form -->
            <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                <form action="{{ route('admin.jobs.update', $job) }}" method="POST" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="6" 
                                      ...
                                      placeholder="Describe the job role, responsibilities, and what you're looking for..." required>{{ old('description', $job->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <textarea name="requirements" id="requirements" rows="4" 
                                      ...
                                      placeholder="List the required skills, qualifications, and experience..." required>{{ old('requirements', $job->requirements) }}</textarea>
                            @error('requirements')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" id="status" 
                                ...
                                <option value="open" {{ old('status', $job->status) == 'open' ? 'selected' : '' }}>Approved (Open)</option>
                                <option value="pending" {{ old('status', $job->status) == 'pending' ? 'selected' : '' }}>Pending Approval</option>
